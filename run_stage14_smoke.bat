...
rem UXM V3.3 Stage-14 smoke runner
rem Only Stage-14 tests are executed. Existing older tests are not regenerated or rerun here.

set "TESTDIR=uxm\tests\v33\stage14"
set "LOGDIR=build\logs\stage14"
set FAIL=0
set COUNT=0
...
call build_native.bat > "%LOGDIR%\stage14_build.log" 2>&1
if errorlevel 1 (
  echo [FAIL_BUILD] build_native.bat
  type "%LOGDIR%\stage14_build.log"
  exit /b 1
)
echo [OK] compiler build

if not exist "%TESTDIR%\*.uxm" (
  echo [FAIL] Stage-14 test klasoru bulunamadi: %TESTDIR%
  exit /b 1
)
...
if not "%FAIL%"=="0" (
  echo [STAGE14_SMOKE_FAIL] %COUNT% test calisti, hata var.
  exit /b 1
)

echo [STAGE14_SMOKE_OK] %COUNT% test basarili.
exit /b 0
