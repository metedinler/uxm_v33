...
Sub MatNDGetV18(ByVal baseAddr As LongInt, ByVal idxBase As LongInt, ByVal dims As LongInt)
    Dim offset As LongInt, stride As LongInt, i As LongInt, dimSize As LongInt, idx As LongInt
    If dims<=0 Or dims>5 Then SetStatus STATUS_DATA_BOUNDS: SetResult STATUS_DATA_BOUNDS: Exit Sub
    offset=0: stride=1
    For i=dims-1 To 0 Step -1
        dimSize=CLngInt(ReadData(baseAddr+i))
        idx=CLngInt(ReadData(idxBase+i))
        If dimSize<=0 Or idx<0 Or idx>=dimSize Then SetStatus STATUS_DATA_BOUNDS: SetResult STATUS_DATA_BOUNDS: Exit Sub
        offset += idx*stride
        stride *= dimSize
    Next
    SetResult ReadData(baseAddr+dims+offset)
    SetLogicFlags ResultValue()
    SetStatus STATUS_OK
End Sub

Sub MatNDSetV18(ByVal baseAddr As LongInt, ByVal idxBase As LongInt, ByVal dims As LongInt, ByVal value As LongInt)
    Dim offset As LongInt, stride As LongInt, i As LongInt, dimSize As LongInt, idx As LongInt
    If dims<=0 Or dims>5 Then SetStatus STATUS_DATA_BOUNDS: SetResult STATUS_DATA_BOUNDS: Exit Sub
    offset=0: stride=1
    For i=dims-1 To 0 Step -1
        ...
        stride *= dimSize
    Next
    WriteData baseAddr+dims+offset, FromSignedValue(value)
    SetResult value And CellMask()
    SetLogicFlags ResultValue()
    SetStatus STATUS_OK
End Sub

Function MatEig2SymV18(ByVal baseAddr As LongInt, ByVal which As LongInt) As LongInt
    Dim a As Double,b As Double,d As Double,tr As Double,disc As Double,l1 As Double,l2 As Double
    a=CDbl(MatGet(baseAddr,0,0)): b=CDbl(MatGet(baseAddr,0,1)): d=CDbl(MatGet(baseAddr,1,1))
    tr=a+d: disc=Sqr((a-d)*(a-d)+4.0*b*b)
    l1=(tr+disc)/2.0: l2=(tr-disc)/2.0
