...
import argparse, csv, datetime, json, re, subprocess, time
...
def main(argv=None) -> int:
    ap=argparse.ArgumentParser(description="UXM Expected Runner V3")
    ap.add_argument("--root",default=".")
    ap.add_argument("--test-dir",default="")
    ap.add_argument("--manifest",default="")
    ap.add_argument("--recursive",action="store_true")
    ap.add_argument("--stage",default="expected_v3")
    ap.add_argument("--out-root",default="expected_results_v3")
    ...
    print(f"UXM Expected Runner V3: tests={len(items)} run_dir={outdir}")
    ...
    for idx,item in enumerate(items,1):
        ...
        code,out,err,sec=run_cmd(f'"{build_one}" "{rel}" -x',root,args.timeout_test)
    ...
    (outdir/"EXPECTED_RUNNER_V3_REPORT.md").write_text("\n".join(["# UXM Expected Runner V3 Summary","",f"- Total selected: {len(items)}",f"- Passed: {passed}",f"- Mismatch: {mismatch}",f"- Build/run fail: {buildfail}",f"- Skipped: {skip}","",f"CSV: `{outdir/'expected_results_v3.csv'}`",f"Mismatches: `{outdir/'mismatches_v3.csv'}`"]),encoding="utf-8")
