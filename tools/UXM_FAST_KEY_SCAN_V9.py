...
def main():
    ap = argparse.ArgumentParser(description="UXM Fast Key Scan V8")
    ...
    (out / "FAST_KEY_REPORT.md").write_text("# UXM Fast Key Report V9\n\n" + json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"[FAST_SCAN_V9] source={summary['source']} bad_rows={len(rows)} unique_bad_keys={len(unique_rows)}")
    print(f"[FAST_SCAN_V9] unique_manifest={out / 'failed_unique_manifest.csv'}")
    return 0
