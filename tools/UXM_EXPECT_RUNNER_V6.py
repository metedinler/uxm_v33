...
def main(argv=None) -> int:
    ap=argparse.ArgumentParser(description="UXM Expected Runner V6")
    ...
    print(f"UXM Expected Runner V6: tests={len(items)} run_dir={outdir}")
    ...
    (outdir/"EXPECTED_RUNNER_V3_REPORT.md").write_text("\n".join(["# UXM Expected Runner V6 Summary","",f"- Total selected: {len(items)}",f"- Passed: {passed}",f"- Mismatch: {mismatch}",f"- Build/run fail: {buildfail}",f"- Skipped: {skip}","",f"CSV: `{outdir/'expected_results_v3.csv'}`",f"Mismatches: `{outdir/'mismatches_v3.csv'}`"]),encoding="utf-8")
